---
title: EDUCATION & CERTS
formal_education:
  - institution: Universitas Gadjah Mada (UGM)
    degree: Bachelor of Philosophy - BA, ( Philosophy )
    date: Aug 2021 - Jul 2025
    gpa: 3.70
    description: ''
    skills:
      - Research
      - Logic
      - Technology
    logo: /abouts/educations/ugm.png
    organizations:
      - role: Project Coordinator (Incubated Startup Core Team)
        name: Innovative Academy (Universitas Gadjah Mada)
        date: Feb 2024 - Aug 2025 · 1 yr 7 mos
        location: Daerah Istimewa Yogyakarta, Indonesia · On-site
        description: 'Participated in an intensive university incubator program, which taught me how to support ongoing project work streams from the ground up in a highly dynamic, fast-paced environment. Helped the core team stay organized and on track by diligently documenting mentor feedback, tracking daily operational tasks, and ensuring all cross-functional members were aligned with the current goals. Navigated constant project pivots, which deeply ingrained in me the importance of embracing failures as lessons, adapting quickly, and continuously pushing the project forward without losing attention to administrative details.'
        skills:
          - Project Coordination
          - Adaptation
          - Editing
          - Stakeholder Management
          - Google Workspace
        logo: /abouts/educations/iaugm.png

      - role: User Experience Researcher
        name: Bantuternak (PT Bantu Ternak Indonesia)
        date: Sep 2021 - Sep 2022 · 5 mos
        location: Daerah Istimewa Yogyakarta, Indonesia · Hybrid
        description: 'Conducted user research and carefully compiled raw feedback into structured insights. Honed my ability to analyze scattered information and turn it into clear reports—a foundational skill highly applicable for market or competitor analysis.'
        skills:
          - Market Research
          - Visualisasi Data
          - Microsoft PowerPoint
          - Time Management
        logo: /abouts/works/bt.png

      - role: Independent entrepreneur program participants
        name: Kampus Merdeka · Internship
        date: Aug 2023 - Dec 2023 · 5 mos
        location: Indonesia
        description: 'Collaborated within a cross-functional team to conduct comprehensive market research and develop a viable business proposal from the ground up (which later evolved into the "Angkut Ternak" project). Gained intensive hands-on experience in executing operational feasibility studies, analyzing market data, and translating raw concepts into structured project documentation. Regularly pitched project initiatives to industry experts and mentors, honing my ability to communicate ideas clearly, accept constructive feedback, and adapt project plans quickly under pressure.'
        skills:
          - Project Coordination
          - Adaptation
          - Editing
          - Stakeholder Management
          - Google Workspace
        logo: /abouts/educations/wmk.png

      - role: Security Coordinator
        name: Banwaslu (Badan Pengawas Pemilu) UGM
        date: Jan 2023 - Feb 2023 · 2 mos
        location: Yogyakarta, Indonesia · On-site
        description: 'Coordinated on-the-ground field operations during a highly dynamic and high-pressure student election period, teaching me how to solve operational problems on the spot.'
        skills:
          - Project Coordination
          - Adaptation
          - Editing
          - Stakeholder Management
          - Google Workspace
        logo: /abouts/educations/banwas.png

      - role: Staff Human Resources Development
        name: Lingkar Studi Filsafat (LSF) Cogito
        date: Sep 2021 - Sep 2022 · 1 yr 1 mos
        location: Daerah Istimewa Yogyakarta, Indonesia · Hybrid
        description: ''
        skills:
          - Attention to Detail
          - Data Entry
          - Administrative Work
          - Microsoft Excel
        logo: /abouts/educations/cogito.png

certifications:
  - title: Introduction to Financial Engineering and Risk Management
    issuer: Columbia University Industrial Engineering and Operations Research
    date: Published Dec 2025
    credential_id: KJOODI51UW8Y
    credential_url: 'https://coursera.org/verify/KJOODI51UW8Y'
    skills:
      - Risk Management
      - Data Analysis
      - Financial Modeling
      - Probability
      - Optimization
    logo: /abouts/educations/columbia.png

  - title: Data Mining in Python
    issuer: University of Michigan - School of Information
    date: Published Jan 2026
    credential_id: 7SX6OI6LDYZP
    credential_url: 'https://coursera.org/verify/7SX6OI6LDYZP'
    skills:
      - Python (Polars & NumPy)
      - Data Analysis
      - Machine Learning
      - Data Visualization
      - Pandas
    logo: /abouts/educations/michigan.png

  - title: Machine Learning
    issuer: DeepLearning.AI
    date: Issued Aug 2025
    credential_id: 6JDMVH42A04B
    credential_url: 'https://coursera.org/verify/7SX6OI6LDYZP'
    skills:
      - Predictive Modeling
      - Machine Learning
      - Model Training
    logo: /abouts/educations/stf.png

  - title: Advanced Learning Algorithms
    issuer: DeepLearning.AI
    date: Issued Jul 2025
    credential_id: UJ5KL3E267U5
    credential_url: ''
    skills:
      - Algorithmic Thinking
      - Problem Solving
      - Neural Networks
    logo: /abouts/educations/stf.png

  - title: 'Supervised Machine Learning: Regression and Classification'
    issuer: DeepLearning.AI
    date: Issued Jun 2025
    credential_id: BED1RL3Z2H5S
    credential_url: ''
    skills:
      - Statistical Data Analysis
      - Trend Analysis
      - Regression Models
    logo: /abouts/educations/stf.png

  - title: Alibaba Cloud Certified Developers
    issuer: Alibaba Cloud
    date: Issued Dec 2024 - Expires Dec 2026
    credential_id: ACCD0119700100015232
    credential_url: ''
    skills:
      - Cloud Computing
      - Python
      - Cloud Architecture
    logo: /abouts/educations/alibaba.png

  - title: Google Data Analytics Professional Certificate
    issuer: Coursera
    date: Dec 2024 - Expires Dec 2026
    credential_id: ACCD0119700100015232
    credential_url: ''
    skills:
      - Data Analysis
      - Data Visualization
      - Analytics
    logo: /abouts/educations/g.png
---
